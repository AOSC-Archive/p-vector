...
def purge_from_dists(base_dir: str, branch_component: List[str]):
    """ Purge the aforementioned branch/component from the database.
        This will delete all the APT assets generated by p-vector for the branch-components.
    """
    dist_dir = base_dir + '/dists'
    for i in branch_component:
        path = PosixPath(dist_dir).joinpath(i)
        logger_gc.info("DELETING branch %s from dist: %s", i, path)



def purge_from_db(db, branch_component: List[str]):
    """ Purge the aforementioned branch/component from the database.
        This will delete all records that:
        - references this branch/component, or
        - references a package that is contained in this branch/component
        with ON DELETE foreign key constraint across all tables, including file listing, package dependency / breakage
        and shared object information.
    """
    cur = db.cursor()
    for i in branch_component:
